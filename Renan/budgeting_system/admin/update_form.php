...
// Fetch all user funds data
$stmt = $pdo->query("SELECT uf.payee, uf.office, uf.address, uf.uacs_code ,uf.amount, uf.responsibility_center, uf.particulars FROM user_funds uf INNER JOIN users u ON uf.user_id = u.id");
...
<body>
    ...
    <?php foreach ($payees as $payee => $funds): ?>
        <div class="payee-table">
            <h3>Payee: <?= htmlspecialchars($payee) ?> <span class="navigation-button" onclick="window.location.href='your_url_here.php?payee=<?= urlencode($payee) ?>'">View Details</span></h3>
            ...
            <table class="second-table">
                <tr>
                    ...
                    <td class="payee1"><?= htmlspecialchars($payee) ?></td>
                </tr>
                <tr>
                    <td class="payee">Office</td>
                    <td class="payee1">
                        <?php foreach ($funds as $user): ?>
                            <div><?= htmlspecialchars($user['office']) ?></div>
                        <?php endforeach; ?>
                    </td>
                </tr>
                <tr>
            ...
            <table class="third-table">
                ...
                <?php foreach ($funds as $user): ?>
                    <tr>
                        <td><?= htmlspecialchars($user['responsibility_center']) ?></td>
                        <td><?= htmlspecialchars($user['particulars']) ?></td>
                        <td>MFO/PAP</td>
                        <td><?= htmlspecialchars($user['uacs_code']) ?></td>
