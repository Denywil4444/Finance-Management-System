...
// Fetch distinct usernames from the admins table
$user_query = "SELECT u.username FROM admins u ORDER BY u.username ASC";
...
$fund_query = "SELECT uf.id, uf.payee FROM admin_funds uf JOIN admins u ON uf.user_id = u.id WHERE u.username = ?";
...
$stmt = $pdo->prepare("SELECT * FROM admin_funds WHERE user_id = (SELECT id FROM admins WHERE username = ?) AND id = ?");
...
<head>
    ...
    <link rel="stylesheet" href="/budgeting_system/assets/css/admin_form.css">
</head>
<body>
    ...
    <button onclick="window.location.href='admin_create.php'">Back to Admin Form</button>
